...
func helpLines(a *App, filter string) []string {
	...
	all := []string{
		...
		"",
		"## Ex commands",
		":qa\tSave all notes and quit ZenNotes",
		":q :bd\tClose the current tab; closing the last tab quits",
		":w :wa :wq :x\tSave, save all, or save and close the current tab",
		...
		"",
		"## Themes and styles",
		"[appearance] in config.toml\tzn draws with the color scheme the desktop app is set to: theme_family, theme_mode and theme_id, custom themes under themes/ and the Quick tweaks colors included",
		":theme dark | light | auto\tMode for this session; auto follows the terminal background. :theme alone flips dark and light within the family",
		":theme <family|id>\tSwitch for this session: a family (apple, gruvbox, catppuccin, github, solarized, one, nord, tokyo-night, kanagawa, black-metal, rose-pine), a variant such as catppuccin-mocha, or a custom theme by folder name. Tab completes",
		":themes (or :colorscheme)\tPick a color scheme from a list that previews each one; Esc puts the old one back",
		"[terminal] theme\tA theme for zn alone, when the terminal should not match the desktop app",
