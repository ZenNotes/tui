...
func helpLines(a *App, filter string) []string {
	...
	all := []string{
		...
		"With Vim mode off, use arrows, Enter and Escape to navigate; F2 opens context actions and Ctrl+G opens commands.",
		"",
		"## Layout",
		...
		leader + " ?\tThis manual",
		"Leader hints appear immediately; timed expires the sequence, sticky keeps it open (vim.which_key_hint_mode).",
		"",
		"## Editor: motions",
		...
		":bind <id> <keys> :unbind <id> :keymaps\tPersistent keymaps; select an action in :keymaps to change its binding",
		":config :reloadconfig\tEdit config.toml externally or reload it; :settings offers searchable controls; external changes load within a second",
		":comments\tAnchored threads: read, reply, resolve/reopen, edit/delete and jump to source; Enter → Read full thread scrolls long replies",
		":files :assets\tAttachments: import, copy/insert links, usage, sort/filter, open, rename/update references, trash/restore",
		":templates\tTemplate manager: preview, edit body/metadata, duplicate, delete/reset a built-in override",
		":complete :table :formatting\tTag/callout/fence completion, table rows/columns/alignment, formatting actions; Ctrl+Space completes in insert mode",
		":bulk :mark\tMark notes and apply open/move/archive/trash/restore; Ctrl+Space marks rows, Shift+Up/Down extends; folder marks include their notes",
		":filters :savefilter :boardoptions\tShared saved task queries; board grouping/root, column labels and card order",
		":conflict\tResolve external edits: reload, keep local, or save a copy; pending/failed saves keep edits recoverable",
		...
		"Filter syntax\tDesktop substring matching: task/note text, !high, #tag, @field:value. Terminal operators require where:, e.g. where: is:open due:today -meeting",
		"Board status\tToday / Upcoming / In progress / Waiting / Done; custom status uses field:status. Folder boards do not move cards between folders.",
		"F2 / :boardoptions\tView actions, saved filters, grouping, folder root, labels and persistent card order",
		"",
		"## Tags",
		...
		"",
		"## Not in the terminal",
		"Workflows, Atlas, sharing, cloud sync, Harper grammar checks and image resizing stay in the desktop app. Optional terminal renderers handle math/diagrams.",
