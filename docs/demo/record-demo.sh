#!/bin/bash
# Records the zn tui demo without touching the screen: the TUI runs in a
...
WORK="$(mktemp -d /tmp/zn-demo.XXXXXX)"
SOCK="zndemo"
COLS=140; ROWS=38
...
mkdir -p "$OUT_DIR"
cp -R "$HERE/vault" "$WORK/Notes"
mkdir -p "$WORK/config" && cp "$HERE/config/config.toml" "$WORK/config/config.toml"
...
asciinema rec --headless --overwrite --window-size ${COLS}x${ROWS} \
  -c "env TERM=xterm-256color LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 tmux -u -L $SOCK attach -t demo" "$OUT_DIR/zn-tui-demo.cast" >"$WORK/asciinema.log" 2>&1 &
REC=$!
sleep 2.5
"$HERE/demo-drive.sh" -L "$SOCK" -t demo:app
wait "$REC"
[ -s "$OUT_DIR/zn-tui-demo.cast" ] || { echo "no recording was produced" >&2; cat "$WORK/asciinema.log" >&2; exit 1; }

agg --font-family "$FONT" --font-size "$FONT_SIZE" --line-height 1.35 --theme "$THEME" \
  --fps-cap 30 --no-loop --last-frame-duration 2 "$OUT_DIR/zn-tui-demo.cast" "$WORK/demo.gif" || exit 1
ffmpeg -v error -y -i "$WORK/demo.gif" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -preset slow -crf 18 -pix_fmt yuv420p -movflags +faststart -r 30 "$OUT_DIR/zn-tui-demo.mp4" || exit 1
ffmpeg -v error -y -i "$WORK/demo.gif" -vf "scale=1280:-2" -c:v libx264 -preset slow -crf 20 -pix_fmt yuv420p -movflags +faststart -r 30 "$OUT_DIR/zn-tui-demo-1280.mp4" || exit 1
ffmpeg -v error -y -ss 4 -i "$OUT_DIR/zn-tui-demo.mp4" -frames:v 1 "$OUT_DIR/zn-tui-demo-poster.png"
ls -la "$OUT_DIR"
